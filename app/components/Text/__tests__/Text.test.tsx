...
it("renders correctly H2 center", () => {
  const tree = create(<Text.H2 center>test</Text.H2>).toJSON();
  expect(tree).toMatchSnapshot();
});
...
it("renders correctly Link", () => {
  const tree = create(<Text.Link url="http://green.me">test</Text.Link>).toJSON();
  expect(tree).toMatchSnapshot();
});

it("open links", () => {
  const spy = jest.spyOn(WebBrowser, "openBrowserAsync");
  const root = create(<Text.Link url="http://green.me">test</Text.Link>).root;
